...
## Шаг 7
Добавьте блок ``||Agent:агент: сделать активной ячейку||`` после внешнего цикла ``||Loops:повторить 4 раза||`` внутрь блока ``||Player:при команде чата||``. Агенту не нужно повторять это, так как это часть нового действия.
